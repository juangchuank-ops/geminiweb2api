...
const ENDPOINTS: EndpointDoc[] = [
  ...
  -d '{
    ...
    response: `data: {"id":"chatcmpl-...","object":"chat.completion.chunk","choices":[{"delta":{"reasoning_content":"先选基准值"}}]}

data: {"id":"chatcmpl-...","object":"chat.completion.chunk","choices":[{"delta":{"content":"快速排序"}}]}
...
data: [DONE]`,
    parameters: [
      ...
      { name: "stream", type: "boolean", note: "是否使用 SSE 流式输出，默认 false" },
      {
        ...
        type: "string",
        note: "思考深度。none / minimal / low 用模型自带的档位，其余取值（medium、high…）强制最深的思考模式",
      },
      { name: "temperature", type: "number", note: "接收但不使用：上游没有这个参数，保留仅为兼容客户端" },
      { name: "max_tokens", type: "number", note: "接收但不使用：输出长度由所选模型决定" },
    ],
  },
  {
    category: "chat",
  ...
  "created": 1758000000,
  "data": [
    { "url": "http://127.0.0.1:8080/media/gen_xxx_0.png", "source_url": "https://lh3.googleusercontent.com/..." }
  ]
}`,
    parameters: [
      { name: "prompt", type: "string", required: true, note: "图像描述" },
      { name: "n", type: "number", note: "接收但不使用：上游按提示词自行决定画几张，返回几张就是几张" },
      { name: "size", type: "string", note: "接收但不使用：尺寸由上游模型决定" },
  ...
  "status": "ok",
  "version": "0.1.0",
  "pool": { "total": 12, "active": 10, "cooldown": 1, "disabled": 0, "invalid": 1, "routable": 10 }
}`,
    parameters: [],
  },
];
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b text-muted-foreground">
                    ...
                    <th className="h-8 px-2 text-left font-normal">{t("docs.type")}</th>
                    <th className="h-8 px-2 text-left font-normal">{t("docs.required")}</th>
                    <th className="h-8 px-2 text-left font-normal">{t("docs.note")}</th>
                  </tr>
                </thead>
                <tbody>
