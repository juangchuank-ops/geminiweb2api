...
import { ExternalLink, RefreshCw, Repeat, ScrollText, Server, Settings2, Shield, Sparkles, Waves } from "lucide-react";
...
type Draft = {
  ...
  // The rotation fields are spelled out with a prefix rather than spread in
  // like the sections above. `enabled` alone would read as the whole gateway's
  // switch rather than one section's, and `timeoutSec` collides outright with
  // the upstream block.
  refreshEnabled: boolean;
  refreshIntervalMin: number;
  refreshGapSeconds: number;
  refreshTimeoutSec: number;
  refreshRetireAfterFailures: number;
};
...
function toDraft(settings: SettingsDTO): Draft {
  ...
  return {
    ...
    refreshIntervalMin: refresh.intervalMin,
    refreshGapSeconds: refresh.gapSeconds,
    refreshTimeoutSec: refresh.timeoutSec,
    refreshRetireAfterFailures: refresh.retireAfterFailures,
  };
}
  ...
  const saveMutation = useMutation({
    mutationFn: (value: Draft) =>
      saveSettings({
        ...
        refresh: {
          enabled: value.refreshEnabled,
          intervalMin: value.refreshIntervalMin,
          gapSeconds: value.refreshGapSeconds,
          timeoutSec: value.refreshTimeoutSec,
          retireAfterFailures: value.refreshRetireAfterFailures,
        },
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <SettingsGroup icon={<Waves />} title={t("settings.groups.upstream")}>
          ...
          <Field label={t("settings.upstream.language")} help={t("settings.upstream.languageHelp")}>
            <Input value={draft.language} placeholder="zh-CN" onChange={(event) => set("language", event.target.value)} />
          </Field>
          <Field label={t("settings.upstream.defaultModel")} help={t("settings.upstream.defaultModelHelp")}>
            <Input
          ...
          <Field label={t("settings.upstream.rotateURL")} help={t("settings.upstream.rotateURLHelp")}>
            <Input
              className="font-mono text-[11px]"
              value={draft.rotateURL}
              placeholder="https://accounts.google.com/RotateCookies"
              onChange={(event) => set("rotateURL", event.target.value)}
            />
          </Field>
          <Field label={t("settings.upstream.requestTimeout")} unit={t("settings.units.seconds")} help={t("settings.upstream.requestTimeoutHelp")}>
            <Input
              type="number"
              min={5}
              max={1800}
          ...
          <Field
            ...
            help={t("settings.upstream.streamIdleTimeoutHelp")}
          >
            <Input
              type="number"
              min={5}
        ...
        <SettingsGroup icon={<Repeat />} title={t("settings.groups.refresh")}>
          <Field label={t("settings.refresh.enabled")} help={t("settings.refresh.enabledHelp")}>
            <Switch checked={draft.refreshEnabled} onCheckedChange={(value) => set("refreshEnabled", value)} />
          </Field>
          <Field label={t("settings.refresh.interval")} unit={t("settings.units.minutes")} help={t("settings.refresh.intervalHelp")}>
            <Input
              type="number"
              min={1}
              max={1440}
              value={draft.refreshIntervalMin}
              onChange={(event) => set("refreshIntervalMin", Number(event.target.value))}
            />
          </Field>
          <Field label={t("settings.refresh.gap")} unit={t("settings.units.seconds")} help={t("settings.refresh.gapHelp")}>
            <Input
              type="number"
              min={0}
              max={120}
              value={draft.refreshGapSeconds}
              onChange={(event) => set("refreshGapSeconds", Number(event.target.value))}
            />
          </Field>
          <Field label={t("settings.refresh.timeout")} unit={t("settings.units.seconds")} help={t("settings.refresh.timeoutHelp")}>
            <Input
              type="number"
              min={5}
              max={300}
              value={draft.refreshTimeoutSec}
              onChange={(event) => set("refreshTimeoutSec", Number(event.target.value))}
            />
          </Field>
          <Field label={t("settings.refresh.retireAfterFailures")} help={t("settings.refresh.retireAfterFailuresHelp")}>
            <Input
              type="number"
              min={1}
              max={100}
              value={draft.refreshRetireAfterFailures}
              onChange={(event) => set("refreshRetireAfterFailures", Number(event.target.value))}
            />
          </Field>
        </SettingsGroup>
        ...
        <SettingsGroup icon={<Settings2 />} title={t("settings.groups.about")}>
          ...
          <Field label={t("settings.about.upstreamURL")}>
            <a
              href={about?.upstreamURL || "https://gemini.google.com/"}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 text-xs text-muted-foreground transition-colors hover:text-foreground"
            >
              {about?.upstreamURL || "https://gemini.google.com/"}
