    ...
    <div className="min-h-screen bg-background">
      <aside className="fixed inset-y-0 left-0 z-30 hidden h-screen w-[288px] flex-col overflow-hidden bg-sidebar px-4 py-6 lg:flex">
        <div className="flex h-7 shrink-0 items-center justify-between px-2.5">
          ...
          {/* Says which upstream surface this gateway wraps. The sidebar is
              288px, so the host will not fit - the tooltip carries it. */}
          <a
            href="https://gemini.google.com/"
            target="_blank"
            rel="noreferrer"
            title="上游：gemini.google.com 网页端"
            className="rounded-full bg-secondary/70 px-1.5 py-0.5 text-[10px] text-muted-foreground transition-colors hover:text-foreground"
          >
            Web
          </a>
        </div>
