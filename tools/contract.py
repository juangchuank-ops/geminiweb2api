...
# A structurally valid cookie. The backend parses __Secure-1PSID out of it and
# derives the account identifier from that value, so a placeholder string would
# be rejected by validation before the route is even reached. The value only has
# to be well-formed — nothing here can tell whether a session is live, and the
# contract check never talks to Google.
CONTRACT_COOKIE = "__Secure-1PSID=g.a000contract0000000000000000; __Secure-1PSIDTS=sidts-contract-value"
...
def main():
    ...
    print("GeminiWeb2API frontend/backend contract check")
    print(f"base = {base}\n")
    ...
    if account_id:
        ...
        probe(base, "account probe", f"/admin/api/accounts/{account_id}/probe",
              "POST", None, token)
        probe(base, "account rotate", f"/admin/api/accounts/{account_id}/refresh",
    ...
    # Cookie rotation. The per-account call goes out to Google, so any non-2xx
    # is still a routed endpoint; what this checks is the path and the verb.
    #
    # `refresh run` is a real sweep, not a dry run: on a populated pool it will
    # rotate every eligible cookie now instead of at the scheduled interval.
    # That is the feature working rather than a side effect to avoid, but it is
    # worth saying out loud, because rotating is a risk-control sensitive call
    # and this tool is otherwise read-only apart from the throwaway account it
    # creates and deletes.
    probe(base, "refresh overview", "/admin/api/refresh", "GET", None, token)
    print("       (refresh run is a real sweep - every eligible cookie gets rotated)")
    probe(base, "refresh run", "/admin/api/refresh/run", "POST", None, token, timeout=180)
    ...
    probe(base, "account import", "/admin/api/accounts/import", "POST",
          {"cookies": "spare----" + CONTRACT_COOKIE.replace("contract", "contract2")}, token)
    ...
    probe(base, "refresh all", "/admin/api/accounts/refresh-all", "POST", None, token, timeout=180)
    # `account cleanup` deliberately does NOT run here. It deletes every account
    # in the requested states, and the throwaway account above is marked invalid
    # (its cookie is a fake, so the probe rejects it) — running cleanup first
    # would delete the very row the delete probe below needs to exist, turning a
    # working route into a misleading 404. It runs at the end instead.
    ...
    # Only now: cleanup sweeps away every account in the given states, and the
    # throwaway account above is one of them.
