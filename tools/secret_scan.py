...
holding a live Gemini JWT. Removing the file and rewriting history was only
...
# `abc123` / `xyz123` are the canonical filler strings people reach for when
# hand-writing a fixture. Suppressing these is not hiding anything: none of
# them is a credential, and flagging them on every run would train the reader
# to ignore the output -- which is worse than not scanning at all.
...
# A stub whose whole content is just the English word for the thing, optionally
# behind a short prefix: `AO-token`, `my-key`, `some-secret`. Real credentials
# are opaque -- base64, hex or random -- so they never look like this.
#
# This exists because of a concrete annoyance: `Session{AccessToken:
# "AO-token"}` in models_test.go tripped `secret-assignment` and, because the
# pre-commit hook refuses on any hit, blocked *every* commit in the repository.
# A hook that cries wolf on a permanent fixture is a hook people learn to
# bypass with --no-verify, which defeats it entirely.
#
# The 10-character prefix cap is what keeps it tight: it will not swallow an
# opaque body that merely happens to end in `-key`.
PLACEHOLDER_STUB = re.compile(
    r"(?i)^[a-z0-9]{0,10}[-_]?(?:token|key|secret|password|passwd|credential|session)$"
)
...
def is_placeholder(value: bytes) -> bool:
    ...
    return bool(
        ...
        or PLACEHOLDER_STUB.match(text)
