...
// A cookie-shaped string for the add-account flow check.
//
// Synthetic on purpose: the flow only needs the value to be non-empty, since
// that is the single thing the dialog requires, and a real session cookie has
// no business being in a test file. The value is obviously a fixture — no live
// account would carry a `synthetic-fixture` literal — so a probe that fails
// against the real upstream is expected and does not stop the account existing.
const SYNTHETIC_COOKIE =
  "__Secure-1PSID=g.a000synthetic-fixture-0000000000000000; " +
  "__Secure-1PSIDTS=sidts-synthetic-fixture";
const USERNAME = "admin";
const TOKEN_KEY = "geminiweb2api:admin-token";
...
// tells the operator to paste a cookie and save. Clicking save therefore
// produced a bare "此项必填" toast and nothing else — every route still rendered
// cleanly, so nothing else in this file noticed.
//
// The counterpart is checked here too: a cookie account saved with an empty
// cookie must be refused, because an empty cookie would silently create a
// guest, and the operator asked for a signed-in account.
//
// It creates an account and deletes it again, so it is re-runnable. The account
// carries a synthetic cookie: the probe that follows creation will fail against
...
async function checkAddAccountFlow(session, adminToken) {
  ...
  await sleep(700);
  await evaluate(session, CLICK("Cookie 登录") + `; "ok"`).catch(() => {});
  await sleep(900);
  ...
  // Saving a cookie account with nothing pasted must be refused. If it were
  // allowed, the account would be created as a guest and the operator would
  // believe they had just added a signed-in account.
  await evaluate(session, CLICK("保存") + `; "ok"`).catch(() => {});
  await sleep(1200);
  const refused = await countAccounts();
  const stillOpenAfterEmptySave = await evaluate(session, `!!document.querySelector("[role=dialog]")`);
  if (refused.total === before.total && stillOpenAfterEmptySave) {
    console.log("  [ok]   an empty cookie is refused, nothing created");
  } else {
    problems.push(
      `an empty cookie was accepted (${before.total} -> ${refused.total}, dialog open: ${stillOpenAfterEmptySave})`,
    );
    console.log(`  [FAIL] an empty cookie was accepted  -> ${before.total} -> ${refused.total}`);
  }

  // Paste a cookie and leave every other field alone.
  await evaluate(
    session,
    `(() => {
      const area = document.querySelector("#account-cookie");
      if (!area) return "no-field";
      const setter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, "value").set;
      setter.call(area, ${JSON.stringify(SYNTHETIC_COOKIE)});
  ...
  const after = await countAccounts();

  if (after.total === before.total + 1) {
    console.log(`  [ok]   cookie-only save created an account  -> ${before.total} -> ${after.total}`);
  } else {
    problems.push(`cookie-only save did not create an account (${before.total} -> ${after.total})`);
    console.log(`  [FAIL] cookie-only save did not create an account  -> ${before.total} -> ${after.total}`);
  }
...
async function main() {
  console.log("GeminiWeb2API real-browser render check");
  ...
  console.log("\nconsole flow: add an account from a pasted cookie alone");
