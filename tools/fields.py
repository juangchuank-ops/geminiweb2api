...
# A structurally valid cookie paste: the backend parses __Secure-1PSID out of
# it and derives the account's identifier from that value, so a placeholder
# string would be rejected before any DTO could be inspected.
FIELDCHECK_COOKIE = "__Secure-1PSID=g.a000fieldchecksession00000000; __Secure-1PSIDTS=sidts-fieldcheck"
...
CHECKS = [
    ...
    ("accounts", "RefreshOverview", "/admin/api/refresh", ""),
    ("accounts", "RefreshSummary", "/admin/api/refresh", "lastSummary"),
...
# written by gateway traffic, and quota only after a successful probe). Fall back
# to comparing against the Go struct's json tags, which still catches a renamed
# or dropped field.
STATIC_SOURCES = {
    "AccountQuota": ("backend/internal/store/types.go", "Quota"),
    "AuditDTO": ("backend/internal/store/types.go", "Audit"),
    "RefreshOutcome": ("backend/internal/refresher/refresher.go", "Outcome"),
    "RefreshSummary": ("backend/internal/refresher/refresher.go", "Summary"),
}
...
def main():
    ...
    print("GeminiWeb2API field-level contract check")
    print(f"base = {base}")
