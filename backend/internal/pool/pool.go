...
func routable(account *store.Account, inflight int, now time.Time) bool {
	...
	// A guest account is routed deliberately and needs no credential. A cookie
	// account needs __Secure-1PSID specifically: without it every request is
	// still answered — as a guest — so the pool would believe it was using the
	// account while the account contributed nothing. That silent downgrade is
	// exactly what the Kind field exists to make impossible.
	if account.Kind != store.KindGuest && account.PSID == "" {
		return false
	}
