...
func (f *fixture) addAccount(t *testing.T, id string, priority int) *store.Account {
	t.Helper()
	return f.addAccountWith(t, id, priority, "seed-"+id, 1)
}

// cookieFor builds a plausible paste. The seed only has to be unique per
// account: __Secure-1PSID is what duplicate detection, the console label and
// the routability check all key on, so two accounts sharing a seed would be
// rejected as duplicates before any test could observe them.
func cookieFor(seed string) string {
	if seed == "" {
		return ""
	}
	return "__Secure-1PSID=g.a000" + seed + "psidvalue000000000; __Secure-1PSIDTS=sidts-" + seed
}

func (f *fixture) addAccountWith(t *testing.T, id string, priority int, seed string, maxConcurrent int) *store.Account {
	t.Helper()
	account := &store.Account{
		ID:     id,
		Name:   id,
		Kind:   store.KindCookie,
		Cookie: cookieFor(seed),
		// The PSID is what makes a cookie account routable. It is normally
		// derived by the store from the paste, and passing it explicitly here
		// keeps the helper honest about what a prepared account looks like.
		PSID:          "g.a000" + seed + "psidvalue000000000",
		Enabled:       true,
		Priority:      priority,
...
// A cookie account with no __Secure-1PSID must never be selected.
//
// This is the single most damaging failure mode the pool has, and it is silent:
// Gemini answers an unauthenticated request happily, as a guest, with a much
// smaller quota and no access to the account's models. An account that is
// picked but carries no session therefore looks like a *working* account while
// contributing nothing — every request succeeds, the success counter climbs,
// and the pool has no signal at all. Refusing to route it is what turns that
// into a visible "no capacity" instead.
func TestAcquireSkipsAccountWithoutPSID(t *testing.T) {
	f := newFixture(t)
	...
	if empty.PSID != "" {
		t.Fatal("fixture no longer models a cookieless account")
	}
}

// A guest account is a deliberate choice, not a half-formed cookie account: it
// carries no credential and is still routable.
func TestAcquireRoutesGuestAccountWithoutCookie(t *testing.T) {
	f := newFixture(t)

	guest := &store.Account{
		ID: "guest", Name: "guest", Kind: store.KindGuest,
		Enabled: true, MaxConcurrent: 1, Status: store.StatusActive,
	}
	if err := f.store.AddAccount(guest); err != nil {
		t.Fatalf("add guest: %v", err)
	}

	lease := f.acquire(t, "")
	defer lease.Release(nil)
	if lease.Account.ID != "guest" {
		t.Fatalf("selected %q, want the guest account", lease.Account.ID)
	}
}

// Losing the PSID is how a revoked cookie presents itself once an operator has
// cleared the field, so the pool has to notice immediately rather than after a
// request comes back degraded.
func TestAcquireSkipsAccountWhosePSIDWasCleared(t *testing.T) {
	f := newFixture(t)
...
func TestAcquireRespectsMaxConcurrent(t *testing.T) {
	f := newFixture(t)
	f.addAccountWith(t, "single", 0, "seed-single", 1)
...
func TestRoutableTreatsZeroMaxConcurrentAsOne(t *testing.T) {
	...
	account := &store.Account{
		...
		Kind:          store.KindCookie,
		Cookie:        cookieFor("x"),
		PSID:          "g.a000xpsidvalue000000000",
		Status:        store.StatusActive,
		MaxConcurrent: 0,
	}
...
// TestRoutableRequiresCredentialOnlyForCookieAccounts pins the one predicate
// that decides whether an account is usable, including the two states that look
// alike and are not.
func TestRoutableRequiresCredentialOnlyForCookieAccounts(t *testing.T) {
	now := time.Now()

	cookie := &store.Account{
		Enabled: true, Kind: store.KindCookie, Cookie: cookieFor("x"),
		PSID: "g.a000xpsidvalue000000000", Status: store.StatusActive, MaxConcurrent: 1,
	}
	if !routable(cookie, 0, now) {
		t.Error("a prepared cookie account should be routable")
	}

	// Same account, cookie field still populated but PSID gone: unroutable.
	// Checking Cookie instead of PSID would call this routable and quietly
	// serve every request as a guest.
	noPSID := *cookie
	noPSID.PSID = ""
	if routable(&noPSID, 0, now) {
		t.Error("a cookie account without __Secure-1PSID must not be routable")
	}

	// A guest is routable precisely because it has no credential to be missing.
	guest := &store.Account{
		Enabled: true, Kind: store.KindGuest, Status: store.StatusActive, MaxConcurrent: 1,
	}
	if !routable(guest, 0, now) {
		t.Error("a guest account should be routable without a cookie")
	}

	// Health gates still apply to both.
	disabled := *cookie
	disabled.Enabled = false
	if routable(&disabled, 0, now) {
		t.Error("a disabled account must not be routable")
	}
	invalid := *cookie
	invalid.Status = store.StatusInvalid
	if routable(&invalid, 0, now) {
		t.Error("an invalid account must not be routable")
	}
}
...
func TestAcquireHonoursContextCancellation(t *testing.T) {
	f := newFixture(t)
	f.addAccountWith(t, "busy", 0, "seed-busy", 1)
	held := f.acquire(t, "")
...
func TestPreferIdleBeatsPriority(t *testing.T) {
	f := newFixture(t)
	f.addAccountWith(t, "important", 1, "seed-important", 5)
	f.addAccountWith(t, "spare", 99, "seed-spare", 5)
...
func TestRoundRobinRotatesAcrossAccounts(t *testing.T) {
	f := newFixture(t)
	f.addAccountWith(t, "a", 0, "seed-a", 5)
	f.addAccountWith(t, "b", 0, "seed-b", 5)
	f.settings.Routing.PreferIdle = false
...
func TestReleaseOnInvalidCredentialDisablesAccount(t *testing.T) {
	...
	lease := f.acquire(t, "")
	lease.Release(gemini.ErrInvalidCredential)
...
func TestSnapshotCopiesInflightCounters(t *testing.T) {
	f := newFixture(t)
	f.addAccountWith(t, "a", 0, "seed-a", 3)
...
func TestConcurrentSchedulingAndStateWrites(t *testing.T) {
	f := newFixture(t)
	for i := 0; i < 4; i++ {
		f.addAccountWith(t, fmt.Sprintf("acct-%d", i), i, fmt.Sprintf("seed-%d", i), 8)
	}
