...
// deriveCookieIdentity fills in the fields that can be read out of a pasted
// cookie, leaving anything the caller already set alone.
//
// Only empty fields are touched, so an operator who renamed an account keeps
// that name, and a PSID the caller extracted with a stricter parser is not
// second-guessed. A guest account has no cookie and is left untouched.
func deriveCookieIdentity(account *Account) {
	if account == nil || account.Cookie == "" {
		return
	}
	psid, psidts, _, _ := gemini.DescribeCredential(account.Cookie)
	if account.PSID == "" {
		account.PSID = psid
	}
	if account.PSIDTS == "" {
	...
	if account.Identifier == "" && psid != "" {
		account.Identifier = gemini.AccountLabel(gemini.Credential{PSID: psid})
	}
	if account.Kind == "" {
		account.Kind = KindCookie
	}
}

// MaskCookie renders a credential safe for display.
//
// The value shown is the __Secure-1PSID rather than the raw paste: the paste
// begins with the cookie *names* (`__Secure-1PSID=…`), which are identical for
// every account, so masking the string as a whole would show eight characters
// that carry no information while hiding the only part that identifies anyone.
func MaskCookie(raw string) string {
	psid := cookieValue(raw, "__Secure-1PSID")
	if psid == "" {
		if raw == "" {
			return ""
		}
	...
	return psid[:8] + "…" + psid[len(psid)-8:]
}

// CookieNamesOf lists which cookies a paste actually contained.
//
// It exists so an operator can see at a glance whether they pasted a session or
// just the one cookie they found first — a distinction that otherwise only
// shows up as every request quietly being answered as a guest.
func CookieNamesOf(raw string) []string {
	var names []string
...
// cookieValue reads one cookie out of a pasted string.
...
func (a *Account) clone() *Account {
	...
	out := *a
	// Quota is the only pointer field left, so the shallow copy above would
	// otherwise share it with the store's own struct and let a caller mutate
	// state it does not hold the lock for.
	if a.Quota != nil {
		quota := *a.Quota
		out.Quota = &quota
	}
...
func (s *Store) AddAccount(account *Account) error {
	s.mu.Lock()
	defer s.unlockAndNotify()
	// Identity is derived here rather than trusted from the caller.
	//
	// The pool decides routability by looking at PSID, and the console shows
	// the identifier, so an account that arrives without either is an account
	// that appears in the list and is never used. Filling them in from the
	// cookie makes that state unreachable instead of merely unlikely.
	deriveCookieIdentity(account)
	// Duplicates are detected on __Secure-1PSID rather than on the whole
	// cookie string. The string changes every time PSIDTS rotates, so comparing
	// it would let the same account be added again after its first refresh —
	// and the pool would then send every request twice from one identity.
	for _, existing := range s.state.Accounts {
		if account.PSID != "" && existing.PSID == account.PSID {
			return ErrAlreadyExists
		}
		if account.PSID == "" && account.Cookie != "" && existing.Cookie == account.Cookie {
			return ErrAlreadyExists
		}
	}
	if account.ID == "" {
